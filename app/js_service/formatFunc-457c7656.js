...
var initFunc = /*#__PURE__*/function () {
  function initFunc() {
    ...
    this.getParentIds = function (parent) {
      ...
      if (parent && parent.id) {
        ids = service.lodash.concat(ids, parent.id);
  ...
  service.createClass(initFunc, [{
    ...
    value: function formatProduct() {
      ...
      service.lodash.forEach(categoryList, function (c) {
        categoryObj[c.id] = _objectSpread({}, c);
      });

      var productObj = {};

      service.lodash.forEach(productList, function (p) {
    ...
    value: function paymentKeyboardAssemble() {
      ...
      var mergeKeys = function mergeKeys(data) {
        ...
        var result = service.lodash.map(data, function (item) {
          var id = item.id,
      ...
      return [].concat(service.toConsumableArray(payment), childrenData);
    } //格式化支付键盘
    ...
    value: function formatMainKeyboard(mainKeyboardData, ProductObj) {
          ...
          if (item.type === 'CATEGORY' || item.type === 'EMPTY') {
            ...
            if (ProductObj[item.object_id]) {
              ...
              if (ProductObj[item.object_id].topping) {
                ...
                ProductObj[item.object_id].topping.map(function (top) {
                  //console.log('top',top)
            ...
            item.has_sku) {
              if (item.sku.length > 0) {
                item.sku = item.sku.map(function (s) {
                  if (ProductObj[s.link_product_id]) {
                    ...
                    if (ProductObj[s.link_product_id].topping) {
                      ProductObj[s.link_product_id].topping.map(function (top) {
                ...
                var visible = service.lodash.filter(item.sku, function (s) {
      ...
      var mainData = [].concat(service.toConsumableArray(ddd), childrenData); //console.log('商品键盘合并之后：', mainData)
